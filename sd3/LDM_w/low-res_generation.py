...
pretrained_vae_path="/shared/s1/lab06/wonyoung/diffusers/sd3/LDM_w/results/E1_wLDM_VQGAN3D/checkpoint-2000000"
pretrained_unet_path="/shared/s1/lab06/wonyoung/diffusers/sd3/LDM_w/results/E1_wLDM_UNET3D/checkpoint-100000"
...
unet3d, vae, = accelerator.prepare(
...
#             #torch.save(x.cpu(), f"{output_dir}/tensor_low-res_image_origin.pth")
...
with torch.no_grad():
    for i in range(num_samples):
        ...
        torch.save(image.cpu(), f"{output_dir}/tensor_low-res_image_1_scale2.pth")
        # cond = torch.tensor([0.1], dtype=torch.float16).to(unet3d.device)
        # image = noise_scheduler.sample(vae=vae_model,
        #                                unet=unet3d_model,
        #                                image_size=int(input_size[1] / vae.config.downsample[1]),
        #                                num_frames=int(input_size[0] / vae.config.downsample[0]),
        ...
        #                                batch_size=1
        #                                )
        # torch.save(image.cpu(), f"{output_dir}/tensor_low-res_image_01_scale2.pth")
...
# raw_slice_images[0].save(f"{output_dir}/low-res_coronal_raw_scale2.gif", save_all=True, append_images=raw_slice_images[1:], duration=200, loop=0)
# recon_slice_images[0].save(f"{output_dir}/low-res_coronal_recon_scale2.gif", save_all=True, append_images=recon_slice_images[1:], duration=200, loop=0)
gen_slice_images[0].save(f"{output_dir}/low-res_coronal_gen_scale2.gif", save_all=True, append_images=gen_slice_images[1:], duration=200, loop=0)
