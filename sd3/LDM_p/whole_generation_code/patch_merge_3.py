...
def adjust_patch_brightness_slice_wise(patch, neighbor_patch, overlap_coords, smoothing=False, alpha=1.0):
    """
    Adjusts the brightness of a patch based on the overlapping region with a neighboring patch,
    performing adjustments slice-wise along the depth axis.

    Args:
        patch (Tensor): The patch to adjust, shape (C, D_p, H_p, W_p).
        neighbor_patch (Tensor): The neighboring patch, shape (C, D_p, H_p, W_p).
        overlap_coords (dict): Coordinates of the overlapping region within both patches.
        smoothing (bool): Whether to apply smoothing to the scaling factors.
        alpha (float): Weighting factor for scaling factors (0 < alpha <= 1).

    Returns:
        Tensor: The adjusted patch.
    """
    ...
    for d in range(patch_overlap.shape[1]):  # Iterate over depth slices
        # Compute mean intensities for the current slice
        mean_patch_slice = patch_overlap[:, d, :, :].mean()
        mean_neighbor_slice = neighbor_overlap[:, d, :, :].mean()

        # Compute scaling factor for the slice
        scaling_factor = mean_neighbor_slice / (mean_patch_slice + 1e-8)

        # Optionally limit the scaling factor
        scaling_factor = torch.clamp(scaling_factor, 0.9, 1.1)

        # Weighted averaging
        scaling_factor = alpha * scaling_factor + (1 - alpha) * 1.0

        scaling_factors.append(scaling_factor.item())

    # Convert scaling factors to tensor
    scaling_factors = torch.tensor(scaling_factors, dtype=torch.float32)

    # Apply smoothing if desired
    if smoothing:
        kernel_size = 5
        padding = (kernel_size - 1) // 2  # For kernel_size=5, padding=2
        smoothing_kernel = torch.ones(kernel_size) / kernel_size
        smoothing_kernel = smoothing_kernel.view(1, 1, -1)  # Shape: (1, 1, kernel_size)

        # Reshape scaling_factors for conv1d
        scaling_factors = scaling_factors.view(1, 1, -1)  # Shape: (1, 1, N)

        # Apply convolution with padding
        scaling_factors_smoothed = torch.nn.functional.conv1d(
            scaling_factors,
            smoothing_kernel,
            padding=padding
        ).squeeze()
        ...
        scaling_factors = scaling_factors_smoothed

    # Apply scaling factors to the slices
    adjusted_patch = patch.clone()
    for i, d in enumerate(range(patch_overlap.shape[1])):
        scaling_factor = scaling_factors[i]
        adjusted_patch[:, coords1['d_start'] + d, :, :] *= scaling_factor

    return adjusted_patch


def adjust_patches_based_on_overlaps_slice_wise(patches, mapping, patch_size, smoothing=False, alpha=1.0):
    ...
    for idx, patch in enumerate(patches):
        ...
        for neighbor_idx in neighbors:
            ...
            if overlap_coords is None:
                continue  # No overlap
            # Adjust brightness slice-wise
            adjusted_patch = adjust_patch_brightness_slice_wise(patch, neighbor_patch, overlap_coords, smoothing, alpha)
...
# Adjust patches with slice-wise adjustment, smoothing, and weighted averaging
adjusted_patches = adjust_patches_based_on_overlaps_slice_wise(
    concat_patches, mapping, patch_size, smoothing=True, alpha=0.8
)
...
concat_1[0].save("/shared/s1/lab06/wonyoung/diffusers/sd3/LDM_p/results/whole_generation/concat_whole_gen1_smooth2.gif", save_all=True, append_images=concat_1[1:], duration=200, loop=0)
