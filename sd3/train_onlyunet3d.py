...
@torch.no_grad()
def log_validation(input_size, test_dataloader, unet3d, noise_scheduler, accelerator, weight_dtype, epoch, num_samples):
    logger.info("Running validation... ")
    #vae = accelerator.unwrap_model(vae)
    unet3d = accelerator.unwrap_model(unet3d)

    image = noise_scheduler.sample(vae=None,
                                   unet=unet3d,
                                   image_size=32,
                                   num_frames=32,
                                   channels=1,
    ...
    images = torch.stack([x.cpu(), image.cpu()], dim=1)
...
def main():
    ...
    # Model memory check
    dummy_input = torch.ones(1, 1, 32, 32, 32).float().to(accelerator.device) # 262,144 for 64^3
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        log_validation(input_size, test_dataloader, unet3d, noise_scheduler, accelerator, weight_dtype, epoch, args.num_samples)
