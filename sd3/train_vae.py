            ...
            tracker.log(
                {
                    "Original (left), Reconstruction (right)": [
                        wandb.Image(torchvision.utils.make_grid(image).permute(1, 2, 0))
...
def main():
    ...
    class UKB_Dataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            # Load the image
            image = np.load(os.path.join(self.data_dir, 'final_array_128_full_' + str(image_name) + '.npy')).astype(np.float16)[:,64,:,:] # (128,128,1)
            #print("image shape:", image.shape)
            image = torch.from_numpy(image).type(torch.float16) ###
            image = image.permute(2, 0, 1) # (1,128,128)
    ...
    if args.use_ema:
        ema_vae.to(accelerator.device, dtype=torch.float16)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(vae):
                ...
                with autocast_ctx:
                    ...
                    if accelerator.num_processes > 1:
                        posterior = vae.module.encode(target).latent_dist # code run this line
                    else:
