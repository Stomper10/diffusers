...
def volume_pairs_generator(original_dir, generated_dir):
    ...
    #generated_files = sorted(os.listdir(generated_dir))[:100] # SET
    generated_files = [file for file in os.listdir(generated_dir) if "A64" in file and "img1" in file]

    i = 0
    for orig_file, gen_file in zip(original_files, generated_files):
        print(i, orig_file.split("/")[4], gen_file)
        ...
        original_volume = np.array(sample["pixel_values"].squeeze(0)) # (128, 128, 128)
                
        # generation
        gen_path = os.path.join(generated_dir, gen_file)
        generated_volume = np.load(gen_path).squeeze(0) # (128, 128, 128)
        ### SET
        image = torch.from_numpy(generated_volume).unsqueeze(0) # (1, 128, 128, 128)
        sample = {"pixel_values": image.to(torch.float16),}
        sample = train_transforms(sample)
        generated_volume = np.array(sample["pixel_values"].squeeze(0)) # (128, 128, 128)
        ...
        if i == 0:
            ...
            image.save("/shared/s1/lab06/wonyoung/diffusers/sd3/LDM/gen_volumes/gif/original_slice.png")  # Save as PNG

            slice_2d = generated_volume[64]
            slice_2d = (slice_2d - slice_2d.min()) / (slice_2d.max() - slice_2d.min()) * 255
            slice_2d = slice_2d.astype(np.uint8)
            image = Image.fromarray(slice_2d)
            image.save("/shared/s1/lab06/wonyoung/diffusers/sd3/LDM/gen_volumes/gif/generated_slice.png")  # Save as PNG
...
generated_dir = "/shared/s1/lab06/wonyoung/diffusers/sd3/LDM/gen_volumes/stage2"
...
# Compute metrics
metrics_results = compute_metrics_for_volumes(origin_paths, generated_dir)
...
def volume_pairs(original_dir, generated_dir):
    ...
    for orig_file, gen_file in zip(original_files, generated_files):
        ...
        if i == 0:
            ...
            slice_2d = slice_2d.astype(np.uint8)
            image = Image.fromarray(slice_2d)
            image.save("/shared/s1/lab06/wonyoung/diffusers/sd3/LDM/gen_volumes/gif/generated_slice_2.png")  # Save as PNG

            slice_2d = generated_volume[:, 64, :]
            slice_2d = (slice_2d - slice_2d.min()) / (slice_2d.max() - slice_2d.min()) * 255
            slice_2d = slice_2d.astype(np.uint8)
            image = Image.fromarray(slice_2d)
            image.save("/shared/s1/lab06/wonyoung/diffusers/sd3/LDM/gen_volumes/gif/generated_slice_3.png")  # Save as PNG

            slice_2d = generated_volume[:, :, 64]
            slice_2d = (slice_2d - slice_2d.min()) / (slice_2d.max() - slice_2d.min()) * 255
            slice_2d = slice_2d.astype(np.uint8)
            image = Image.fromarray(slice_2d)
            image.save("/shared/s1/lab06/wonyoung/diffusers/sd3/LDM/gen_volumes/gif/generated_slice_4.png")  # Save as PNG
...
origin_list, gen_list = volume_pairs(origin_paths, generated_dir) # (100,1,128,128,128) tensor
...
print(fid.compute()) # feature=2048 metatensor(79.9834) / feature=64 metatensor(1.3275)

fid = FrechetInceptionDistance(feature=64)
fid.update(org_slice_3, real=True)
fid.update(gen_slice_3, real=False)
print(fid.compute()) # feature=2048 metatensor(111.9057) / feature=64 metatensor(1.3887)

fid = FrechetInceptionDistance(feature=64)
fid.update(org_slice_4, real=True)
fid.update(gen_slice_4, real=False)
print(fid.compute()) # feature=2048 metatensor(83.7298) / feature=64 metatensor(1.2256)
