...
class UKB_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            image = nib.load(image_path)  # (128,128,128,1)
        except FileNotFoundError:
            raise FileNotFoundError(f"Image file not found: {image_path}")
        
        image = image.get_fdata()
        image = torch.from_numpy(image).unsqueeze(0).unsqueeze(0) # Stays on CPU # (1, 1, 182, 218, 182)
...
def main():
    ...
    unet3d = Unet3D(
        dim=int(input_size[1] / vae.config.downsample[1]), # 20
        cond_dim=4, # cond_tensor
        dim_mults=dim_mults,
        channels=int(vae.config.embedding_dim), # 4
    ...
    with accelerator.main_process_first():
        if args.data_dir is not None: # args.test_data_dir is not None and args.data_dir is not None:
            train_dataset = UKB_Dataset(args.data_dir, args.train_label_dir, transform=train_transforms)
            valid_dataset = UKB_Dataset(args.data_dir, args.valid_label_dir, transform=train_transforms)
