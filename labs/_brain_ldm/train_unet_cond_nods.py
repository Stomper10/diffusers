...
@torch.no_grad()
def log_validation(input_size, vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch):
    ...
    for i in range(len(args.validation_prompts)):
        ...
        image = rgb_to_grayscale_3d(image.unsqueeze(dim=0)) ###
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            tracker.log(
                {
                    "validation Axial": [
                        wandb.Image(image[:, :, image.shape[2] // 2, :, :].squeeze(), caption=f"{i}: {args.validation_prompts[i]}")
                        for i, image in enumerate(images)
                    ],
                    "validation Sagittal": [
                        wandb.Image(image[:, :, :, image.shape[3] // 2, :].squeeze(), caption=f"{i}: {args.validation_prompts[i]}")
                        for i, image in enumerate(images)
                    ],
                    "validation Coronal": [
                        wandb.Image(image[:, :, :, :, image.shape[4] // 2].squeeze(), caption=f"{i}: {args.validation_prompts[i]}")
                        for i, image in enumerate(images)
                    ]
                }
            )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple example of a Unet training script.")
    parser.add_argument(
...
def main():
    ...
    class UKB_Dataset(Dataset):
        ...
        def tokenize_captions(text, is_train=True):
            # captions = []
            ...
            #         )
            inputs = tokenizer(
                str(text), max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
            )
        ...
        def __getitem__(self, index):
            ...
            sample["label"] = torch.tensor(label).type(torch.float16)
            sample["input_ids"] = self.tokenize_captions(self.text)
            del image, label
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}") ### indent in one more from here
        ...
        if accelerator.is_main_process: ### indent in one more from here
